	...
	async execute(interaction) {
        ...
        else if (interaction.isButton())
        {
            switch(interaction.customId)
            {
                ...
                case "ReverifBTN":
                {
                    await interaction.reply({ content: `:ok: Permintaan sedang diproses, mohon tunggu!`, ephemeral : true});
                ...
                case "ReAssignBTN":
                {
                    ...
                    mysqlConnection.getConnection(async function (err, con) {
                        try {
                            ...
                            interaction.member.roles.add("1504914355110678731").catch(() => {
                            ...
                            await interaction.editReply({ content: `:white_check_mark: UCP anda adalah **${row[0].ucp}** dan proses refund role telah berhasil!`, ephemeral : true});
                            con.release();
                        } catch(e) {
                            if(err) console.log(err);
                            console.log(e);
        ...
        else if (interaction.isModalSubmit())
        {
            switch(interaction.customId)
            {
                case "AccRegisterModal":
                {
                    ...
                    await interaction.reply({ content: `:ok: Pendaftaran sedang diproses, mohon tunggu!`, ephemeral : true});
                            ...
                            if(row.affectedRows > 0)
                            {
                                ...
                                const embeol = new EmbedBuilder()
                                    ...
                                    .addFields(
                                        ...
                                        {name : "**IP Address:**", value : "```139.59.245.181```", inline : true},
                                        {name : "**Port:**", value : "```7008```", inline : true}
                                              )
                                ...
                                interaction.member.setNickname(`Warga | ${username}`).catch(() => {
                                    console.log(`Cannot change nickname to ${username}`);
                                });
                                con.release();
